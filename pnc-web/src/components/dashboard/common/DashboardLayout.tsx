...
const getSidebarItems = (role: UserRole, currentPath: string = '') => {
  // Base items without the dashboard link to avoid /dashboard/dashboard
  const baseItems = [
  ...
  // Add dashboard as the first item for all roles
  const items = roleSpecificItems[role] || baseItems;
  return [
            ...
            <div className="relative">
              ...
              >
                <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center">
                  <span className="text-emerald-700 font-medium">{user?.full_name?.charAt(0)?.toUpperCase() || 'U'}</span>
                </div>
                {sidebarOpen && (
                  <>
