...
import { Zap, Droplet, Users, BookOpen, Leaf, ChevronRight, Globe } from 'lucide-react';
...
const SubProjectDetailPage: React.FC<SubProjectDetailPageProps> = async ({ params }) => {
  const resolvedParams = await params;
  const project = PROJECTS_DATA.find(p => p.link === resolvedParams.projectId);
  const subProject = project?.subProjects?.find(sp => sp.link === resolvedParams.subProjectId);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-teal-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href={`/projects/${resolvedParams.projectId}`} className="text-emerald-600 hover:underline">
